...
export type RunAgentInput = {
  messages: ChatTurn[];
  agent: Pick<AgentProfile, "name" | "title" | "instructions">;
  memory: Pick<MemoryFile, "filename" | "kind" | "content">[];
  knowledge: Pick<KnowledgePage, "title" | "kind" | "tags" | "content">[];
  workspace: Pick<WorkspaceFile, "path" | "content">[];
  skills: Pick<Skill, "name" | "instructions">[];
  mode?: "chat" | "dream";
};

export type RunAgentResult =
  | { ok: true; content: string; traces: ToolTrace[]; patches: CowPatches }
    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          ...
          content: { type: "string" },
          kind: { type: "string", enum: ["global", "daily", "dream"] },
        },
    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          title: { type: "string" },
          content: { type: "string" },
          kind: { type: "string", enum: ["concept", "source", "entity"] },
...
function scoreMatch(hay: string, query: string) {
...
function runTool(
  ...
  rawArgs: string,
  ctx: {
    memory: RunAgentInput["memory"];
    knowledge: RunAgentInput["knowledge"];
    workspace: RunAgentInput["workspace"];
    patches: {
      memory: MemoryFile[];
  ...
  if (name === "search_memory") {
    const q = args.query ?? "";
    const ranked = ctx.memory
      .map((f) => ({ f, s: scoreMatch(`${f.filename}\n${f.content}`, q) }))
      .filter((x) => x.s > 0)
      .sort((a, b) => b.s - a.s)
      .slice(0, 4);
  ...
  if (name === "write_memory") {
    ...
    const kind = (args.kind as MemoryFile["kind"]) || (filename === "MEMORY.md" ? "global" : filename === "DREAM.md" ? "dream" : "daily");
  ...
  if (name === "search_knowledge") {
    const q = args.query ?? "";
    const ranked = ctx.knowledge
      .map((p) => ({ p, s: scoreMatch(`${p.title} ${p.tags.join(" ")} ${p.content}`, q) }))
      .filter((x) => x.s > 0)
      .sort((a, b) => b.s - a.s)
      .slice(0, 5);
  ...
  if (name === "write_knowledge") {
    ...
    const page: KnowledgePage = {
      id: uid(),
      title,
      kind: (args.kind as KnowledgePage["kind"]) || "concept",
      tags: (args.tags ?? "")
        .split(",")
        .map((t) => t.trim())
        .filter(Boolean),
...
function systemPrompt(input: RunAgentInput) {
  ...
  if (input.mode === "dream") {
    return `You are CowAgent Deep Dream. Distill the provided memory and recent conversation into:
...
async function chatOnce(apiKey: string, messages: OaiMsg[], withTools: boolean) {
...
export const runAgent = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<RunAgentResult> => {
    ...
    const patches: CowPatches & {
      memory: MemoryFile[];
      knowledge: KnowledgePage[];
      workspace: WorkspaceFile[];
      tasks: ScheduledTask[];
    } = { memory: [], knowledge: [], workspace: [], tasks: [] };
    ...
    try {
      for (let round = 0; round < 5; round++) {
        ...
        if (msg.content?.trim() && (calls.length || round === 0)) {
          traces.push({
            id: uid(),
            kind: "thought",
            name: "Thought",
        ...
        for (const call of calls) {
          ...
          const traceId = uid();
          let output = "";
          let status: ToolTrace["status"] = "ok";
          try {
          ...
          messages.push({
            ...
            content: clip(output, 4000),
          });
        }
      }

      const last = await chatOnce(apiKey, messages, false);
      return {
        ok: true,
        content: (last?.content ?? "Reached the tool-step limit.").trim(),
        traces,
        patches,
      };
    } catch (err) {
